...
export default function ComparePage() {
  ...
  const details = useQueries({
    queries: selected.map((entry) => ({
      queryKey: ["compare-detail", entry._id],
      queryFn: async () =>
        (await axios.get(`${apiBaseUrl}/pokemon/${entry._id}`)).data,
    })),
  });
  const records = details.map((query, index) => query.data || selected[index]);
  const add = (entry) =>
    setSelected((current) =>
      current.some((item) => item._id === entry._id) || current.length >= 4
        ? current
        : [...current, entry],
    );
  const remove = (id) =>
  ...
  useEffect(() => {
    ...
    const key = selected
      ...
      .sort()
      .join(":");
    if (key !== recordedSelection.current) {
      recordedSelection.current = key;
      recordComparison(selected);
    }
  }, [recordComparison, selected]);
  return (
    <div className="pokedex-screen compare-page">
      <div className="boot-screen__scanlines" />
      <PageHeader
        eyebrow="Strategy tools"
        title="Compare Pokémon"
        description="Place two to four Pokémon side by side. Strongest numeric values are highlighted so the tradeoffs are visible at a glance."
      />
      <PokemonSearchPicker
        selectedIds={selected.map((entry) => String(entry._id))}
        onSelect={add}
        disabled={selected.length >= 4}
        label={`Add Pokémon (${selected.length}/4)`}
      />
      <section className="compare-selection terminal-panel">
        {selected.length ? (
          selected.map((entry) => (
            <article key={entry._id} className="terminal-panel">
              ...
              <h2 className="sys-prompt">{entry.pokemon.name}</h2>
              <div>
          ...
          <EmptyState title="Choose Pokémon to compare">
            Search the catalog above to add up to four records.
          </EmptyState>
        )}
      </section>
      {selected.length >= 2 && (
        <section
          className="comparison-table terminal-panel"
          style={{ "--compare-columns": records.length }}
          aria-label="Pokémon comparison"
        >
          <div className="comparison-table__header">
            <span>Stat</span>
            {records.map((record) => (
              <strong key={record._id}>{record.pokemon?.name}</strong>
            ))}
          </div>
          {comparisonRows.map(([label, key]) => {
            const values = records.map(
              (record) => Number(record.pokemon?.[key]) || 0,
            );
            ...
            return (
              <div className="comparison-table__row" key={key}>
                ...
                {values.map((value, index) => (
                  <b
                    key={`${key}-${index}`}
                    className={value === max && max > 0 ? "is-best" : ""}
                  >
                    {value || "—"}
                  </b>
                ))}
          ...
          <div className="comparison-table__row comparison-table__abilities">
            <span>Abilities</span>
