...
export default function SightingsMapPage() {
  ...
  return <div className={`sightings-page ${fullScreen ? 'sightings-page--fullscreen' : ''}`}><PageHeader eyebrow="Exploration" title={title} description="Filter the canonical sightings dataset by time and radius, then switch between clustered markers and a heatmap." actions={<button className="button button--quiet" onClick={() => setFullScreen(!fullScreen)}>{fullScreen ? 'Exit fullscreen' : 'Fullscreen map'}</button>} />
    <section className="map-controls"><label>From<input type="date" value={dateFrom} onChange={(event) => setDateFrom(event.target.value)} /></label><label>To<input type="date" value={dateTo} onChange={(event) => setDateTo(event.target.value)} /></label><label className="radius-control">Radius <input type="range" min="1" max="200" value={radius} onChange={(event) => setRadius(Number(event.target.value))} disabled={!radiusEnabled} /><span>{radius} km</span></label><button className={`button ${radiusEnabled ? 'button--primary' : 'button--quiet'}`} onClick={requestLocation}>{radiusEnabled ? 'Location active' : 'Use my location'}</button><button className={`map-mode ${mode === 'clusters' ? 'is-active' : ''}`} onClick={() => setMode('clusters')}>Clusters</button><button className={`map-mode ${mode === 'markers' ? 'is-active' : ''}`} onClick={() => setMode('markers')}>Markers</button><button className={`map-mode ${mode === 'heatmap' ? 'is-active' : ''}`} onClick={() => setMode('heatmap')}>Heatmap</button></section>
    {mapError && <ErrorBanner title="Map display note" message={mapError} />}
    <section className="sighting-summary"><article><small>Total matching</small><strong>{summary.total.toLocaleString()}</strong></article><article><small>Visible on map</small><strong>{summary.visible.toLocaleString()}</strong></article><article><small>Latest sighting</small><strong>{formatDate(summary.latest_date)}</strong></article><article><small>Top hotspot</small><strong>{hotspot ? `${hotspot.count} points` : '—'}</strong></article></section>
    <div className="map-layout"><section className="map-stage"><div ref={mapNode} className="google-map" aria-label="Google Maps sightings map" />{!mapsKey && <div className="map-fallback"><strong>Map key not configured</strong><span>The interactive list and filters still work. Add your restricted key to `frontend/.env.local` to enable Google Maps.</span></div>}</section><aside className="sighting-sidebar"><div><h2>Visible sightings</h2><span>{points.length} loaded</span></div>{points.slice(0, 80).map((item) => <button key={item.id} className={selected?.id === item.id ? 'is-selected' : ''} onClick={() => focus(item)}>{item.image_url && <img src={item.image_url} alt="" />}<span><strong>{item.pokemon_name}</strong><small>{formatDate(item.date)}</small><i>{item.types.map((type) => <TypeBadge key={type} type={type} compact />)}</i></span></button>)}{!points.length && <p>No sightings match the current filters.</p>}</aside></div>
  </div>;
}
